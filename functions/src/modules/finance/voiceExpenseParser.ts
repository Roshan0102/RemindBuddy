 ...
 */
export const parseVoiceExpensesWithAI = functions.runWith({ timeoutSeconds: 60, memory: "1GB" }).https.onCall(async (data, context) => {
    if (!context.auth) {
    ...
    if (!userGeminiKey) {
        throw new functions.https.HttpsError(
            'failed-precondition',
            'Please configure your Gemini API Key in AI Keys Settings to use Voice Expense Logging.'
        );
    }
    ...
    const combinedCategories = Array.from(new Set([
        ...defaultCategories,
        ...(Array.isArray(customCategories) ? customCategories.map((c: any) => String(c).trim()).filter(Boolean) : [])
    ]));

    const accountsListStr = Array.isArray(accounts) && accounts.length > 0
        ? accounts.map((a: any) => `- ${a.name || a.id}`).join('\n')
...
"${transcript.trim()}"`;
    ...
    try {
        const aiResponse = await callGeminiAPI(geminiPayload, {
        ...
        const rawList = Array.isArray(parsedResult?.transactions)
            ? parsedResult.transactions
            : (Array.isArray(parsedResult) ? parsedResult : []);

        const validTransactions = rawList.map((tx: any) => ({
