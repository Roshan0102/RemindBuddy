...
async function fetchGoldPriceFromTOI(): Promise<number | null> {
    try {
        ...
        if (!finalPrice) throw new Error("Price element not found in TOI");
        return finalPrice;
    } catch (e) {
...
exports.scheduledGoldFetch = functions.pubsub.schedule('0 * * * *')
    ...
    .onRun(async (context) => {
        ...
        // 1. Fetch Price
        let currentPrice = await fetchGoldPriceFromBankBazaar();
        console.log(`   > BankBazaar Price: ${currentPrice}`);

        if (!currentPrice) {
            console.log(`   > Priority fetch failed. Falling back to TOI...`);
            currentPrice = await fetchGoldPriceFromTOI();
        ...
        let oldPriceStr: number | null = null;

        if (latestDoc.exists) {
            oldPriceStr = latestDoc.data()?.price;
        } else {
            ...
            if (yesterdayDoc.exists) oldPriceStr = yesterdayDoc.data()?.price;
        }
