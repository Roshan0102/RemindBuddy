...
exports.onCollaborationRequestUpdated = functions.firestore
    ...
    if (newData.status === 'approved' && oldData.status !== 'approved') {
        ...
        if (!senderUid || !receiverUid || !itemId || !type)
            return;
        try {
            ...
            for (const billDoc of billsSnap.docs) {
                ...
                for (const notifRule of notifications) {
                    ...
                    if (todayStr === targetDateStr && currentTimeStr === notifTime) {
                        ...
                        const notifRef = db.collection("users").doc(uid).collection("notification_logs").doc(notifKey);
                        const notifDoc = await notifRef.get();
                        if (notifDoc.exists)
                            continue;
                        const userAccountDoc = await db.collection("usernames").where("uid", "==", uid).limit(1).get();
                        if (!userAccountDoc.empty) {
                            ...
                            if (token) {
                                ...
                                await logNotification(uid, title, body, "BILL_REMINDER");
                                await notifRef.set({ sentAt: admin.firestore.FieldValue.serverTimestamp() });
...
exports.sendJobApplicationEmail = functions.runWith({ timeoutSeconds: 60, memory: "512MB" }).https.onCall(async (data, context) => {
    ...
    try {
        ...
        if (!userEmail || !appPassword) {
            throw new functions.https.HttpsError('failed-precondition', 'Your Gmail App Password is not configured. Please enter your Gmail & App Password in Job Assistant Settings.');
        }
